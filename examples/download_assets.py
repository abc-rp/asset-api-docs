...
def parse_args():
    ...
    parser.add_argument(
        "--uprn",
        help="Single UPRN value (literal string)",
    )
    parser.add_argument(
        "--uprns",
        help="Comma-separated list of UPRN values",
    )
    parser.add_argument(
        "--csv",
    ...
    parser.add_argument(
        "--types",
        help="Comma-separated list of asset type IRIs to filter by type, e.g. did:rgb-image,did:lidar-pointcloud-merged",
    )
    parser.add_argument(
        "--db-url",
...
"""
    ...
    # Backtrack from resource to Observation/Uprn
    if args.sensor:
        where_clauses += [
...
def main():
    args = parse_args()
    # Gather UPRNs from --uprn, --uprns, and --csv
    uprn_list = []
    if args.uprn:
        uprn_list.append(args.uprn.strip())
    if args.uprns:
        uprn_list.extend(u.strip() for u in args.uprns.split(',') if u.strip())
    if args.csv:
        uprn_list.extend(load_uprns_from_csv(args.csv))
    # Dedupe
    uprn_list = list(dict.fromkeys(uprn_list))
    if not uprn_list:
        raise RuntimeError("Must provide at least one UPRN via --uprn, --uprns, or --csv")
